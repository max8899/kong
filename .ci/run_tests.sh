...
if [ "$KONG_TEST_DATABASE" == "postgres" ]; then
    export TEST_CMD="bin/busted $BUSTED_ARGS,cassandra,memory"
    ...
    export TEST_CMD="bin/busted $BUSTED_ARGS,postgres,memory"
else
...
if [ "$TEST_SUITE" == "dbless" ]; then
    eval "$TEST_CMD" spec/02-integration/02-cmd \
                     ...
                     spec/02-integration/04-admin_api/15-memory_spec.lua
fi
